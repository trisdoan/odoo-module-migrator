...
class ResPartner(models.Model):
    ...
    def test_improve_transalation_with_line_break(self):
        raise ValidationError(
            _(
                "The '%s' is empty or multilinestringgg. It should have a non-null value."
            )
            % "Price"
        )
    ...
    def test_improve_translation_with_brackets_and_line_break(self):
        return _("User %(name)s has %(items)s items") % {
            "name": "Dev",
            "items": 5,
        }
    ...
    def test_improve_translation_with_format_multi_params_has_end_comma(self):
        return _('User {name} has "acb" {items} items').format(
            name="Dev",
            items=5,
        )
